@foreach($contact_persons as $contact)
    <tr>
        ...
        <td>
            ...
            @if(hasPermission('edit_staff_contact_person'))
                ...
                @endif
                <form
                    action='{{route('admin.staff.contact_person.destroy',['staff'=>$staff->id,'contact_person'=>$contact->id])}}'
                    method='POST' class='delete_contact_person_form'>
                    @csrf
                    @method("DELETE")
                    @if(hasPermission('delete_staff_contact_person'))
