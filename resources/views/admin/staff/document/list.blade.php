@foreach($documents as $document)
    <tr>
        ...
        <td>
            <a href="{{route('admin.staff.document.download',['staff'=>$staff->id,'document'=>$document->id])}}">Click
                To Download</a>
        </td>
        <td>
            <div class='btn-group' role='group'>
            @if(hasPermission('edit_staff_document'))
                ...
                @endif
                <form
                    action='{{route('admin.staff.document.destroy',['staff'=>$staff->id,'document'=>$document->id])}}'
                    method='POST' class='delete_document_form'>
                    @csrf
                    @method("DELETE")
                    @if(hasPermission('delete_staff_document'))
