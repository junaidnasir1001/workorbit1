@foreach($contact_persons as $contact)
    <tr>
        ...
        <td>
            ...
            @if(hasPermission('edit_site_contact_person'))
                ...
                @endif
                <form
                    action='{{route('admin.site.contact_person.destroy',['site'=>$site->id,'contact_person'=>$contact->id])}}'
                    method='POST' class='delete_contact_person_form'>
                    @csrf
                    @method("DELETE")
                    @if(hasPermission('delete_site_contact_person'))
